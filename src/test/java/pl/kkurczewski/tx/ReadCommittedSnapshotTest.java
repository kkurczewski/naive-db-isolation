...
class ReadCommittedSnapshotTest {

    @Test
    void isolation_is_readCommittedSnapshot_and_phantomRow_occurred_1() {
        var table = Table.of(1, 2, 3);

        try (var tx = ReadCommittedSnapshot.begin(table)) {
    ...
    @Test
    void isolation_is_readCommittedSnapshot_and_phantomRow_occurred_2() {
        var table = Table.of(1, 2, 3);
    ...
    @Test
    void isolation_is_readCommittedSnapshot_and_phantomRow_occurred_3() {
        var table = Table.of(1, 2, 3);
        ...
        try (var tx = ReadCommittedSnapshot.begin(table)) {
            ...
            assertThat(singleQuery.result()).isNotEqualTo(table.getRecords().size());
            assertThat(table.getRecords().toList().stream().map(Record::getValue)).containsExactly(4, 1, 2, 3);
        }
    }
    ...
    void isolation_is_readCommittedSnapshot_and_row_move_occurred_1() {
        ...
        try (var tx = ReadCommittedSnapshot.begin(table)) {
            ...
            assertThat(singleQuery.result()).isEqualTo(6);
    ...
    void isolation_is_readCommitted_and_row_move_occurred_2() {
        ...
        try (var tx = ReadCommitted.begin(table)) {
            SumQuery singleQuery = new SumQuery();
